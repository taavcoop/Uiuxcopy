...
const DATA: Section[] = [
  {
    ...
    items: [
      ...
      },
      {
        id: 'calendar',
        title: 'تقویم کاری',
        description: 'تعریف تعطیلات رسمی و روزهای کاری سال 1403.',
        status: 'mandatory',
        icon: Calendar,
      },
      {
      ...
      },
      {
        ...
        description: 'افزودن کاربران و تخصیص آن ها به گروه های کاری.',
        status: 'mandatory',
        icon: Users,
        path: '/employees',
      },
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              ...
              <p className="text-slate-400 text-sm max-w-lg leading-relaxed">
                برای استفاده کامل از امکانات پنل، لطفا موارد زیر را بررسی و تکمیل نمایید. این موارد برای محاسبه دقیق حقوق و دستمزد ضروری هستند.
              </p>
            </div>
            ...
            <div className="w-full md:w-64 bg-slate-900/50 backdrop-blur-sm p-4 rounded-2xl border border-white/5">
              ...
              <div className="mt-3 text-[10px] text-slate-500 text-center">5 مورد از 20 مورد تکمیل شده است</div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {DATA.map((section) => (
            <div key={section.id} className="bg-slate-900/40 border border-white/5 rounded-2xl p-2 sm:p-4 overflow-hidden">
              <SectionHeader section={section} isOpen={expandedSections[section.id]} onToggle={() => toggleSection(section.id)} />
                  ...
                  >
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 p-2 pt-0">
                      {section.items.map((item) => (
                        <TaskCard key={item.id} item={item} onClick={() => item.path && navigate(item.path)} />
                      ))}
                    </div>
                    <div className="h-2" />
                  </motion.div>
